...
var require_util = __commonJS({
  "node_modules/@middy/util/index.js"(exports, module2) {
    ...
    var getInternal = async (variables, request) => {
      ...
      const errors2 = values.filter((res) => res.status === "rejected").map((res) => res.reason.message);
      if (errors2.length)
        throw new Error(JSON.stringify(errors2));
...
// src/functions/daily-report/controller/dailyReportController.ts
var dailyReportController_exports = {};
__export(dailyReportController_exports, {
  add: () => add,
  get: () => get
});
module.exports = __toCommonJS(dailyReportController_exports);
...
var makeErrorResponseBody = (err) => {
  const msg = err instanceof Error ? err.message : "";
  const code = err.code !== void 0 ? err.code : "";
  return {
    code,
    message: msg
  };
};
var ErrorResponse = async (err) => {
  const statusCode = err.statusCode !== void 0 ? err.statusCode : 500;
...
// src/libs/repositories/awsImpl/dynamoDbImpl.ts
var import_dynamodb = require("aws-sdk/clients/dynamodb");
var import_process = require("process");
var region = "ap-northeast-1";
var options = void 0;
if (import_process.env.IS_OFFLINE) {
  options = {
    region: "localhost",
...
var ddbClient = new import_dynamodb.DocumentClient(options);
var DailyReportRepositoryByAWSDynamoDBImpl = class {
  constructor() {
    this.findByPrimaryKey = async (userId, reportDate) => {
    ...
    this.findByTeamAndMonth = async (teamId, reportYearMonth) => {
      return [];
    };
    this.findByProjectAndMonth = async (projectId, reportYearMonth) => {
      return [];
    };
    ...
    this.createDailyReportSortKey = async (reportDate, lineNo) => {
    ...
    this.splitDailyReportSortKey = async (reportDateAndLineNo) => {
      const reportDate = new Date(Number(reportDateAndLineNo.substring(0, 4)), Number(reportDateAndLineNo.substring(4, 2)), Number(reportDateAndLineNo.substring(6, 2)));
      const lineNo = Number(reportDateAndLineNo.substring(8));
      return [reportDate, lineNo];
    };
    this.convertAttributeMapToEntity = async (items) => {
      const firstItem = items[0];
      let reportDetail = [];
      for (const item of items) {
        const splitReportDateAndLineNo = await this.splitDailyReportSortKey(item.reportDateAndLineNo);
        reportDetail.push({
    ...
    this.convertEntityToAttributeMap = async (entity, detail) => {
      const tmp = {
...
// src/libs/usecases/dailyReportUsecase.ts
var DailyReportUsecase = class {
  constructor() {
    this.getDailyReport = async (userId, reportDate) => {
      return await this.dailyReportRepository.findByPrimaryKey(userId, reportDate);
    };
    this.addDailyReport = async (userId, reportDate, reportDetail) => {
...
// src/functions/daily-report/controller/dailyReportController.ts
var addDailyReport = async (event) => {
  const logger = await new Logger("Controller").getLogger();
  logger.info(`event: ${JSON.stringify(event)}`);
  try {
    const userId = getMandatoryPathParameter(event, "userId");
    const reportYear = Number(getMandatoryPathParameter(event, "reportYear"));
    const reportMonth = Number(getMandatoryPathParameter(event, "reportMonth"));
    const reportDay = Number(getMandatoryPathParameter(event, "reportDay"));
...
var getDailyReport = async (event) => {
  ...
  try {
    ...
    logger.debug(`${userId} - ${reportDate}`);
    const uc = new DailyReportUsecase();
    const result = await uc.getDailyReport(userId, reportDate);
    return formatJSONResponse({ result });
  } catch (err) {
    return await ErrorResponse(err);
  }
};
var get = middyfy(getDailyReport);
var add = middyfy(addDailyReport);
